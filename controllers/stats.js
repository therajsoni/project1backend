...
export const getPieCharts = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) {
    ...
    const [
      ...
      allOrderPromise,
      User.find({}).select(["dob"]),  // dob this.dob select in user for age virtual model
...
export const getBarCharts = TryCatch(async (req,res,next) => {
  ...
  if(myCache.has(key)){
    ...
    const productCounts = getChartData({
      length:6,today,docArr : products
    })

    const usersCounts = getChartData({
      length:6,today,docArr : users
    })

    const ordersCounts = getChartData({
      length:12,today,docArr : orders
    })
